...
class PRReviewer:
    ...
    def review_diff(self, changed_lines_map: Dict[str, Set[int]]) -> PRReviewResult:
        ...
        for target in self.config.watch:
            findings = self._review_target(target, changed_lines_map)
        ...
        summary = self._generate_summary(all_findings, breaking_count, dep_count)
        verdict = (
    ...
    def _generate_summary(
        self, findings: List[Finding], breaking_count: int, dep_count: int
    ) -> str:
        ...
        if not findings:
            return (
                ...
                "| :--- | :--- | :--- |\n"
                f"| `{len(self.config.watch)} target(s)` | ✅ Clean | 0 risks found |\n"
            )
        ...
        lines.extend(
            [
                ...
                "\n---",
                "*Generated by [Hound API Watchdog](https://github.com/8dazo/hound)*",
            ]
        )
